...
def build_likelihood(params: NamedParameters):
    ...
    """
    for i in range(5):
        # Define the spectroscopic galaxy clustering tracer:
    ...
    # For the cross-correlations of spectroscopic clustering:
    for i in range(5):
        statistics[f"tp_spec_lens{i}_lens{i}"] = TwoPoint(
            source0=sources[f"lens_spec{i}"],
            source1=sources[f"lens{i}"],
            sacc_data_type="spce_density_xi",
        )

    for i in range(5):
        statistics[f"tp_spec_lens{i}_src{i}"] = TwoPoint(
            source0=sources[f"lens_spec{i}"],
            source1=sources[f"src{i}"],
            sacc_data_type="spec_shearDensity_xi",
        )
    # Auto and cross-correlated TwoPoint object of scpectroscopic clustering
    for i in range(5):
        for j in range(i, 5):
            statistics[f"spec_lens{i}_spec_lens{j}"] = TwoPoint(
                source0=sources[f"lens_spec{i}"],
                source1=sources[f"lens_spec{j}"],
                sacc_data_type="spec_auto",
            )
